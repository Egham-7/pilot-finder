...
export const firecrawlResearchTool = createTool({
  ...
  description:
    "Perform deep research on a topic using KlavisAI Firecrawl MCP server to gather comprehensive information from multiple sources",
    ...
    try {
      ...
      const results = Array.isArray(searchResults)
        ? searchResults.map((result: any) => ({
            ...
            content:
              result.content || result.markdown || "No content available",
